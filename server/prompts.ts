...
/**
 * Sección de estructura/cronograma del despliegue para el prompt del tutor.
 * - TUTOR_SIG_CRONOGRAMA_TEXTO: texto propio del curso (reemplaza la sección).
 * - TUTOR_SIG_CRONOGRAMA = "0"/"false": omite la sección por completo.
 * - Por defecto: cronograma del Diplomado CESMAG.
 */
export function buildCronogramaSection(env: NodeJS.ProcessEnv): string {
  const custom = env.TUTOR_SIG_CRONOGRAMA_TEXTO?.trim();
  if (custom) {
    return `\n\nESTRUCTURA DEL CURSO:\n- ${custom}`;
  }

  const visible = (env.TUTOR_SIG_CRONOGRAMA || "1").trim().toLowerCase();
